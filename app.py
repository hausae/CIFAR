...
h2 { font-size:1.5rem !important; font-weight:700 !important; color:var(--dark) !important;
...
/* browse-files button — light style so it doesn't go dark */
...
/* ── Stat boxes ── */
.stat-box  { text-align:center; padding:16px 10px; }
.stat-val  { font-size:2rem; font-weight:800; color:var(--dark); line-height:1; }
.stat-lbl  { font-size:10px; font-weight:600; color:var(--muted); text-transform:uppercase;
             letter-spacing:.07em; margin-top:4px; }
.stat-box-dark { text-align:center; padding:16px 10px;
    background:var(--dark2); border-radius:var(--r-sm); }
.stat-box-dark .stat-val { color:#fff; }
.stat-box-dark .stat-lbl { color:var(--light); }

/* ── Dark bento (pure HTML, no widget inside) ── */
.dark-card {
    background: var(--dark2); border-radius: var(--r); padding: 22px 24px;
...
.arch-row { display:flex; align-items:center; gap:10px;
            padding:7px 0; border-bottom:1px solid rgba(212,168,130,0.2); }
.arch-row:last-child { border-bottom:none; }
.arch-dot  { width:7px; height:7px; border-radius:50%;
             background:var(--gold); flex-shrink:0; }
.arch-name { font-size:11px; font-weight:600; color:#fff !important; width:90px; flex-shrink:0; }
.arch-val  { font-size:11px; color:var(--light) !important;
...
.pred-lbl   { font-size:10px; font-weight:700; letter-spacing:.1em;
              text-transform:uppercase; color:var(--light) !important; margin-bottom:4px; }
.pred-name  { font-size:30px; font-weight:800; color:#fff !important;
              text-transform:capitalize; line-height:1; }
.pred-conf  { font-size:14px; font-weight:500;
              color:var(--gold-lt) !important; margin-top:6px; }
...
th { background:var(--pale) !important; color:var(--dark) !important;
     padding:8px 14px; font-size:11px; font-weight:700; text-transform:uppercase;
     letter-spacing:.05em; border:1px solid var(--border); }
...
.vok  { display:inline-block; padding:3px 10px; border-radius:20px; font-size:11px;
        font-weight:700; background:var(--pale); color:var(--brown);
...
/* ── Spinner ── */
[data-testid="stSpinner"] * { color:var(--brown) !important; }
...
<div style="background:#fff;border:1px solid var(--border);border-radius:var(--r);
  ...
  <div style="display:flex;gap:7px;flex-wrap:wrap">
    <span class="chip chip-dark">CNN · TensorFlow</span>
...
<div class="dark-card">
  ...
  <div class="arch-row"><div class="arch-dot" style="background:#fff"></div><div class="arch-name">Output</div><div class="arch-val">10 units, Softmax</div></div>
</div>
""", unsafe_allow_html=True)

st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)

# ── Model load + stat strip ───────────────────────────────────
...
<div style="background:#fff;border:1px solid var(--border);border-radius:var(--r);padding:20px 24px;margin-top:8px">
  ...
  <p style="font-size:13px;margin-bottom:10px">Run this in your Jupyter notebook, then copy the file into the <code>cifar10-webapp</code> folder:</p>
  <code style="background:var(--dark);color:var(--gold-lt);padding:8px 16px;border-radius:8px;
               display:inline-block;font-size:13px">model.save("cifar10_model.keras")</code>
</div>""", unsafe_allow_html=True)
    st.stop()

# Stat strip
s1, s2, s3, s4 = st.columns(4, gap="small")
for col, val, lbl, dark in [
    (s1, "10",  "Classes",    False),
    (s2, "32²", "Input Size", False),
    (s3, "20",  "Epochs",     False),
    (s4, "✓",   "Model Ready",True),
]:
    ...
    <div class="pred-conf">Confidence: {confidence:.1f}%</div>
        ...
        with st.container(border=True):
            ...
            for i in sorted_idx:
                ...
                fill   = "cbar-fill" if is_top else "cbar-dim"
                cls_cl = "cbar-cls-b" if is_top else "cbar-cls"
                bars += f"""
<div class="cbar">
  <div class="cbar-head">
    <span class="{cls_cl}">{CLASS_EMOJIS[cls]} {cls}</span>
...
st.markdown(
    ...
    'Upload one sample image per class. Screenshot each prediction for your report.</p>',
    unsafe_allow_html=True
)
st.markdown("<div style='height:6px'></div>", unsafe_allow_html=True)
...
val_cols    = st.columns(5, gap="small")
val_results = []

for i, (col, label, true_cls) in enumerate(zip(val_cols, val_labels, val_classes)):
    with col:
        with st.container(border=True):
            ...
            if vfile:
                ...
                badge   = "vok" if correct else "verr"
                icon    = "✅" if correct else "❌"
                st.markdown(
                    f'<span class="{badge}">{icon} {vcls.capitalize()}</span>',
                    unsafe_allow_html=True
                )
                st.caption(f"{vconf:.1f}%")
                val_results.append({
                    "true": true_cls, "predicted": vcls,
                    "confidence": vconf, "correct": correct
                })
            else:
                st.markdown(
                    '<div style="height:70px;display:flex;align-items:center;'
                    'justify-content:center;font-size:22px;opacity:.25">📷</div>',
                    unsafe_allow_html=True
                )
...
if val_results:
    ...
    mc[2].metric("Accuracy",  f"{accuracy:.0f}%")
    ...
    for j, r in enumerate(val_results, 1):
        ...
        rows.append(
            f"| {j} | {CLASS_EMOJIS[r['true']]} {r['true'].capitalize()} "
            f"| {CLASS_EMOJIS.get(r['predicted'],'')} {r['predicted'].capitalize()} "
            f"| {r['confidence']:.1f}% | {badge} |"
        )
    st.markdown("\n".join(rows))
    ...
    cm = [
        "| True \\ Pred | " + " | ".join(c.capitalize() for c in u_cols) + " |",
    ...
    for tr in u_rows:
        ...
        for pr in u_cols:
            ...
            cell = (f"**{cnt}** ✅" if tr==pr else f"**{cnt}** ❌") if cnt else "·"
            cells.append(cell)
...
st.markdown(
    ...
    'ICT120 Final Project · CIFAR-10 CNN Image Classifier · Step 6 Web Deployment</p>',
    unsafe_allow_html=True
)
